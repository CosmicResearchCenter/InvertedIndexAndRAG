...
def split_file(file_path:str,splitterModel:SplitterModel=SPPLITTER_MODEL)->List[Document]:
    docs:List[Document] = []
    # 判断文件类型
    if file_path.lower().endswith(".txt"):
        ...
        textSplitter = TextSplitter(SPPLITTER_MODEL=splitterModel)
        docs = textSplitter.split(state_of_the_union)
        return docs
    elif file_path.lower().endswith(".pdf"):
        print("pdf file")
        # pdf文件
        loader = PDFSplitter(file_path=file_path,SPPLITTER_MODEL=splitterModel)
        docs = loader.split()
        # print(docs)
    elif file_path.lower().endswith(".doc"):
        loader = DocxSplitter(file_path=file_path,splitter_model=splitterModel)
        docs = loader.split()
