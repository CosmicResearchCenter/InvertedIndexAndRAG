...
# 启动容器
log "启动前端容器"
if docker run -d -p 2024:80 --name $frontend_container_name $frontend_image_name; then
...
log "启动后端容器"
if docker run -d -p 9988:9988  --name $backend_container_name $backend_image_name --network=chat2network; then
